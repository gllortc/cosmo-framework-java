...
public class UserRegisterPage extends Page
{
   ...
   public PageContext loadPageEvent(PageContext pc, HttpServletRequest request, HttpServletResponse response)
   {
       ...
       * try { // Obtiene el usuario PostgreSqlAuthenticationImpl auth =
       * (PostgreSqlAuthenticationImpl)
       * AuthenticationFactory.getInstance(pc.getWorkspace()); User user =
       * auth.add(HttpRequestUtils.getValue(request, "id", ""));
       * 
