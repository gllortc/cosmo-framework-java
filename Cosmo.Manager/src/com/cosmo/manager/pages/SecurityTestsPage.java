...
/**
 * Página ejecuta todos los tests de seguridad para comprobar el correcto
 * funcionamiento del módulo de seguridad de Cosmo.
 * 
 * @author Gerard Llort
 */
   ...
   public PageContext loadPageEvent(PageContext pc, HttpServletRequest request, HttpServletResponse response)
   {
      ...
      if (getWorkspace().isValidUserSession())
      {
         ...
         xhtmlRoles.appendHeadder(Icon.render(Icon.ICON_IMAGE_GROUP) + " Rols d'usuari", 4).appendParagraph(
               "La següent llista conté els rols que té l'usuari:");
         ...
         try
         {
            ...
            xRolList
                  ...
                  .appendParagraph(
                        "La següent llista mostra els rols efectius de l'usuari. S'agafa la llista complerta de rols i un per un es comprova per l'usuari a través la API de seguretat.")
         ...
         xhtmlAct.appendHeadder(Icon.render(Icon.ICON_IMAGE_SHARE) + " Permisos de l'usuari", 4).appendParagraph(
               "La següent llista conté les activitats sobre les que l'usuari té permisos especificats:");
         ...
         else
         {
            ...
            if (!getUserSession().isSuperUser())
            {
               msgAct.setMessage("L'usuari " + XhtmlControl.formatBold(getUserSession().getCurrentUser().getLogin())
                     + " no té permís d'execució per cap activitat.");
            }
            else
            {
               msgAct.setMessage("L'usuari " + XhtmlControl.formatBold(getUserSession().getCurrentUser().getLogin())
                     + " té permís d'execució per totes les activitats (SuperUser).");
            }
         }
         ...
         try
         {
            ...
            xhtmlActLst
                  ...
                  .appendParagraph(
                        "La següent llista mostra els permisos efectius de l'usuari. S'agafa la llista complerta d'activitats i una per una es comprova per l'usuari a través la API de seguretat.")
                  .appendUnorderedList(lst, "alt");
         }
         catch (AuthorizationException ex)
         {
